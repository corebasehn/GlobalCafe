...
async function main() {
  const tables = ['rol', 'permiso', 'rol_permiso', 'usuario', 'usuario_rol', 'log_sistema'];
  for (const table of tables) {
    try {
      const pk = table === 'usuario' ? 'id' : 
                 (table === 'rol_permiso' ? 'id_rol_permiso' : 
                 (table === 'usuario_rol' ? 'id_usuario_rol' : 
                 (table === 'log_sistema' ? 'id_log' : `id_${table}`)));
