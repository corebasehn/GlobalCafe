...
import { AnimatedModal, BasicModal, Fullscreen, ModalGrid, ModalInsideModal, OptionalSize, ScrollingContent, StaticBackdrop, TooltipModal, VARYINGMODAL, VerticallyCentered, VerticallyScrollCentered } from "../../common/Modalfunctionality";

interface ComponentProps { }

const ModalCloses: FC<ComponentProps> = () => {
  ...
  return (
    <Fragment>
      <Pageheader title="MODAL & CLOSES" heading="Advanced UI" active="Modals" />
        ...
        <Col xl={4}>
          <Card className="custom-card">
            ...
            <div className={`card-footer ${isHidden[4] ? '' : 'd-none'} border-top-0`}>
              <pre><code className='language-javascript'>
                {`
function MyVerticallyCenteredScrollingModal(props: any) {
    return (
        <Modal
            {...props}
            aria-labelledby="contained-modal-title-vcenter"
...
export function OptionalSize() {
    ...
    const handleShowModal = (size: string): void => {
        switch (size) {
            case 'xl':
                setShowXlModal(true);
                break;
...
export function VARYINGMODAL() {
    ...
    const [show, setShow] = useState(false);
    const [selectedModal, setSelectedModal] = useState<ModalType>({ id: 0, title: '', color: '', inputfield: '' });
